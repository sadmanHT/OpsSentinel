...
ORDER_MODE = ToolOrderVariant(
    os.environ.get("PHASE8_TOOL_ORDER", ToolOrderVariant.FREE.value)
)
OUTPUT_PATH = Path(
    os.environ.get(
        "PHASE8_TOOL_ORDER_ARM_OUTPUT",
        f"phase8-tool-order-{ORDER_MODE.value}.json",
    )
)
...
async def main() -> None:
    ...
    cell = next(
        item for item in plan.cells if item.configuration.tool_order == ORDER_MODE
    )
    assert cell.configuration.architecture == ArchitectureVariant.EXPLICIT_PLANNER
    assert cell.configuration.tool_order == ORDER_MODE
    ...
    assert len(records) == len(TOOL_ORDER_SCENARIO_IDS) == 10
    ...
    for record in records:
        ...
        assert health["tool_order"] == ORDER_MODE.value
        assert health["tool_order_controlled"] is True
        provider = str(health["provider"])
        assert "tool-order-controlled-v1" in provider
