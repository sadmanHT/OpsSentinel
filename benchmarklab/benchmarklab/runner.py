...
from benchmarklab.models import BenchmarkCatalog, BenchmarkRunArtifact, ScenarioSpec, ServiceName
...
class BenchmarkRunner:
    """Launch benchmark scenarios without exposing hidden labels to the agent."""
    ...
    async def _inject_fault(self, scenario: ScenarioSpec, index: int) -> None:
        ...
        if response.status_code >= 400:
            raise BenchmarkRuntimeError(
                f"fault injection failed for {scenario.scenario_id}: HTTP {response.status_code}"
            )
    ...
    async def _prepare_scenario(self, scenario: ScenarioSpec) -> None:
        events: list[tuple[float, str, int]] = []
        events.extend((fault.offset_seconds, "fault", index) for index, fault in enumerate(scenario.faults))
        events.extend(
        ...
        for offset, event_type, index in events:
            ...
            if event_type == "fault":
                ...
                await self._run_stimulus(scenario, index)
            previous_offset = offset

    async def _run_stimulus(self, scenario: ScenarioSpec, index: int) -> None:
    ...
    async def run(self, scenario: ScenarioSpec, *, benchmark_version: str = "1.0.0") -> BenchmarkRunArtifact:
        await self._restore_all()
        try:
            await self._prepare_scenario(scenario)
